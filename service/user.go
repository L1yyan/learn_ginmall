...
func (service *UserService) Register(ctx context.Context) serializer.Response {
	...
	//创建用户
	err = userDao.CreateUser(&user) //传指针比较好，不会发生拷贝
	if err != nil {
		code = e.Error
	}
	return serializer.Response{
...
// Post 头像更新
func (sercive *UserService) Post(ctx context.Context, uId uint, file multipart.File, fileSize int64) serializer.Response {
	code := e.Success
	var user *model.User
	var err error
	userDao := dao.NewUserDao(ctx)
...
func (service *SendEmailService) Send(ctx context.Context, uId uint) serializer.Response {
	code := e.Success
	var address string
	var notice *model.Notice //绑定邮箱 修改密码的模板通知
...
func (service *ValidEmailService) Valid(ctx context.Context, token string) serializer.Response {
	...
	var email string
	var password string
	var operation_type uint
	code := e.Success
	//验证token
	if token == "" {
		code = e.InvalidParams
	} else {
		claims, err := util.ParseEmailToken(token) //TODO:已经parse过了，为什么还要parse
		if err != nil {
			code = e.ErrorAuthToken
			...
			operation_type = claims.OperationType
		}
	}
	if code != e.Success {
		return serializer.Response{
	...
	if operation_type == 1 {
		...
		//TODO:修改密码 token不是本身信息的加密吗，为什么直接把本身的密码setpassword了，而不是从表单中提取
